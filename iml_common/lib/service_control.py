...
class ServiceControl(object):
    ...
    def _retried_action(self, action, action_codes, validate, error_message, retry_count, retry_time, validate_time):
    ...
    def start(self, retry_count=5, retry_time=1, validate_time=5):
        return self._retried_action(
            lambda: self._start(),
            self.ActionCode(
                self.ServiceState.SERVICESTARTING, self.ServiceState.SERVICESTARTED, self.ServiceState.SERVICESTARTERROR
            ),
            lambda: self.running,
    ...
    def stop(self, retry_count=5, retry_time=1, validate_time=5):
        return self._retried_action(
            lambda: self._stop(),
            self.ActionCode(
                self.ServiceState.SERVICESTOPPING, self.ServiceState.SERVICESTOPPED, self.ServiceState.SERVICESTOPERROR
            ),
    ...
    def register_listener(cls, service, callback):
        ...
        if callback not in cls.callbacks[service]:
            ...
            raise RuntimeError("callback already registered for service %s (%s)" % (service, str(callback)))
    ...
    def unregister_listener(cls, service, callback):
        ...
        try:
            ...
            raise RuntimeError(
                "callback function is not registered for service %s and cannot be "
...
class ServiceControlEL7(ServiceControl):
    ...
    def _applicable(cls):
        return (
            util.platform_info.system == "Linux"
            and cls.platform_use == util.platform_info.distro_version_full.split(".")[0]
        )
